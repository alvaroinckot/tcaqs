...
def extract_info(info):
    ...
    # Extract Imbuements information
    imbuements_table = soup.select('#Imbuements td td td')
    imbuements_count = len(imbuements_table) - 1  # Subtract 1 for header
    
    # Extract detailed imbuements list
    imbuements_list = []
    try:
        imbuement_rows = soup.select('#Imbuements .TableContent tr')[1:]  # Skip header
        for row in imbuement_rows:
            if 'IndicateMoreEntries' not in row.get('class', []):
                cells = row.select('td')
                if cells:
                    imbuement_name = cells[0].get_text().strip()
                    if imbuement_name:
                        imbuements_list.append(imbuement_name)
    except Exception as e:
        print(f"Warning: Could not extract detailed imbuements: {e}")

    # Extract Charms information
    charms_table = soup.select('#Charms td td td')
    charms_count = int(len(charms_table) / 2 - 1) if len(charms_table) > 0 else 0
    
    # Extract detailed charms list with costs
    charms_list = []
    try:
        charm_rows = soup.select('#Charms .TableContent tr')[1:]  # Skip header
        for row in charm_rows:
            cells = row.select('td')
            if len(cells) >= 2:
                charm_cost = cells[0].get_text().strip()
                charm_name = cells[1].get_text().strip()
                if charm_cost and charm_name:
                    charms_list.append({
                        'cost': charm_cost,
                        'name': charm_name
                    })
    except Exception as e:
        print(f"Warning: Could not extract detailed charms: {e}")

    # Extract Completed Quest Lines
    quest_lines = []
    try:
        quest_rows = soup.select('#CompletedQuestLines .TableContent tr')[1:]  # Skip header
        for row in quest_rows:
            if 'IndicateMoreEntries' not in row.get('class', []):
                cells = row.select('td')
                if cells:
                    quest_name = cells[0].get_text().strip()
                    if quest_name:
                        quest_lines.append(quest_name)
    except Exception as e:
        print(f"Warning: Could not extract quest lines: {e}")

    # Extract Mount information (regular mounts)
    mounts_list = []
    try:
        mount_icons = soup.select('#Mounts .CVIcon')
        for icon in mount_icons:
            title = icon.get('title', '').strip()
            if title:
                mounts_list.append(title)
    except Exception as e:
        print(f"Warning: Could not extract mounts: {e}")

    # Extract Store Mounts information
    store_mounts_list = []
    try:
        store_mount_icons = soup.select('#StoreMounts .CVIcon')
        for icon in store_mount_icons:
            title = icon.get('title', '').strip()
            if title:
                store_mounts_list.append(title)
    except Exception as e:
        print(f"Warning: Could not extract store mounts: {e}")

    # Extract Outfits information (regular outfits)
    outfits_list = []
    try:
        outfit_icons = soup.select('#Outfits .CVIcon')
        for icon in outfit_icons:
            title = icon.get('title', '').strip()
            if title:
                outfits_list.append(title)
    except Exception as e:
        print(f"Warning: Could not extract outfits: {e}")

    # Extract Store Outfits information
    store_outfits_list = []
    try:
        store_outfit_icons = soup.select('#StoreOutfits .CVIcon')
        for icon in store_outfit_icons:
            title = icon.get('title', '').strip()
            if title:
                store_outfits_list.append(title)
    except Exception as e:
    ...
    return {
        ...
        'imbuements': imbuements_count,
        'charms': charms_count,
        # New detailed extraction fields
        'imbuements_list': imbuements_list,
...
def save_batch_to_db(data_batch, db_path='characters_v2.db'):
    if not data_batch:
        return 0
    ...
    for data in data_batch:
        if data:
            # Convert list/dict data to JSON strings for storage
            values.append((
